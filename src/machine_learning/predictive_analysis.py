...
def resize_input_image(img):
    ...
    try:
        ...
        st.error(
            "Image shape file not found. Please ensure `image_shape.pk1` "
            "exists in `outputs/v4`."
            )
        raise
...
def load_model_and_predict(my_image):
    ...
    try:
        ...
        st.error(f"Model file not found at: {model_path}."
                 " Please upload the file to `outputs/v4`.")
        raise
    except Exception as e:
    ...
    try:
        ...
        target_map = {
            v: k
            for k, v in {'Healthy': 0, 'Powdery Mildew': 1}.items()
        }
