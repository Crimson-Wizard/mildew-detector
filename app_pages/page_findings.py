...
def page_findings_body():
    ...
    if st.checkbox("Difference between average and variability image"):
        ...
        st.warning(f"We notice a small colour pigment difference in when comparing the average and variability images")
        st.image(avg_healthy, caption='Healthy - Average and Variability')
        st.image(avg_powdery, caption= 'Powdery Mildew - Average and Variability')
        st.write("---")
        
    # checkbox for average healthy vs powder mildew leaf traits
    if st.checkbox("Differences in leaft traits"):
        diff_between_avgs = plt.imread(f"outputs/avg_diff_powdery_mildew_vs_healthy.png")
        st.warning(f"We notice a small differnece in shape between average images")
